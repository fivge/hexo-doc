...
|i_CTRL-N|	CTRL-N		  插入光标前标识符的下一个匹配
|i_CTRL-P|	CTRL-P		  插入光标前标识符的上一个匹配
...
|i_CTRL-T|	CTRL-T		  在当前行首插入一个 shiftwidth 的缩进
|i_CTRL-D|	CTRL-D		  从当前行首删除一个 shiftwidth 的缩进
...
|<|	  N  <{motion}	将动作 {motion} 覆盖的所有行向左移动 shiftwidth
|<<|	  N  <<		将 N 行向左移动 shiftwidth
|>|	  N  >{motion}	将动作 {motion} 覆盖的所有行向右移动 shiftwidth
|>>|	  N  >>		将 N 行向右移动 shiftwidth
...
|:g|	   :[range]g[lobal]/{pattern}/[cmd]
			对 [range] 内所有匹配 {pattern} 的行执行 Ex 命令 [cmd]
			   (缺省: ":p")
|:g|	   :[range]g[lobal]!/{pattern}/[cmd]
			对 [range] 内所有_不_匹配 {pattern} 的行执行 Ex 命令
			   [cmd] (缺省: ":p")
|:so|	   :so[urce] {file}
			从文件 {file} 读入 Ex 命令
|:so|	   :so[urce]! {file}
...
|c_CTRL-V|	CTRL-V {char}	   按本义插入 {char}
...
|c_<Up>|	<Up>/<Down>	   搜索以当前命令开始的较早/较晚的命令行
|c_<S-Up>|	<S-Up>/<S-Down>	   从命令行历史中重放较早/较晚的命令
|c_CTRL-G|	CTRL-G		   'incsearch' 激活时下次匹配
|c_CTRL-T|	CTRL-T		   'incsearch' 激活时前次匹配
...
|:stop|	  :st[op][!]		挂起 Vim 或开始一个新外壳。如果置位了 'aw' 选
				   项并没有给出 [!] 则写入缓冲区
